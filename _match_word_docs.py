"""
Match source tables to Word files using the pre-collected list from the network share.
"""
import os, json

WORD_LIST_FILE  = r"c:\Users\sveliza\Documents\bb_repos\feature-store-bb\_word_files_list.txt"
OUTPUT_FILE     = r"c:\Users\sveliza\Documents\bb_repos\feature-store-bb\_find_word_docs_output.txt"

# ---------------------------------------------------------------------------
# Consolidated source tables (no saiku)
# ---------------------------------------------------------------------------
consolidated = {
...
# ---------------------------------------------------------------------------
# Distinct source tables (no saiku)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
PREFIXES = [
    "dwe_e_", "dwh_h_", "dwh_r_", "dwh_t_", "dlh_h_",
    "dwe_", "dwh_", "dw_", "dlh_",
]
...
# ---------------------------------------------------------------------------
# Load pre-collected word file list
# ---------------------------------------------------------------------------
print(f"Loading word files list from {WORD_LIST_FILE} ...")
word_files = []
with open(WORD_LIST_FILE, encoding="utf-8-sig") as f:
    for line in f:
...
print(f"Loaded {len(word_files)} Word files.\n")

# ---------------------------------------------------------------------------
# Match each source table
# ---------------------------------------------------------------------------
result = {}
...
for st, paths in found.items():
    sn = get_search_name(st)
    if isinstance(paths, list):
        log(f"\n  {st}  [search: '{sn}']")
        for p in paths:
            log(f"    → {p}")
    else:
        log(f"  {st}  [search: '{sn}']  →  {paths}")
